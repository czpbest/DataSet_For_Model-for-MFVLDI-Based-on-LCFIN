...
CONFIG = {
    ...
    "save_path": r"./classic_compare_output",
    "backbones": ["resnet50", "efficientnet_b0", "mobilenet_v3", "our_model"], 
}
...
class OurModel(nn.Module):
    """我们的完整模型：EfficientNet-B3 + SEBlock + MLP-Mixer"""
    def __init__(self, num_classes=12):
        ...
        self.mixer_layers = nn.ModuleList([
            MLPMixerLayer(dim=512, seq_len=100) for _ in range(4)
        ])
...
def get_model(backbone_name, num_classes):
    ...
    elif backbone_name == "our_model":
        model = OurModel(num_classes)
    else:
        raise ValueError("仅支持 resnet50 / efficientnet_b0 / mobilenet_v3 / our_model")
